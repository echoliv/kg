import Fresh_song_v1_1.computeList
...
object recommend_new {
  def main(args: Array[String]) {
    ...
    */
    ...
    val specialRecPath = "file\\ctrrecommend.txt"
    val recommendHistoryPath = "file\\goodSongidPath.txt"
    val playHistoryPath = "file\\grouprec.txt"
    val outputPath1 = "file\\v7.txt"
    val preUserSongPath = "file\\badSongidPath.txt"
    val themeRecPath ="file\\incrementDataPath.txt"
    val languageAudioRecPath ="file\\playHistoryPath.txt"
    val languageRecPath = "file\\productFeaturePath.txt"
    val singerRecPath ="file\\recommendHistoryPath.txt"
    val behaviorRecPath ="file\\similar_song_featursaveAsTextFilee"
    val recnum = 200
    val simmaxnum = 100
    val maxnum = 350
    ...
    val recSpecial1 = preUserSong.join(specialData1).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._3,x._4)}.sortBy(_._2).reverse.take(120).map{x => x._1}
    ...
    val recTheme2 = preUserSong.join(themeData2).map{x =>
      ...
      val rank = info(2)
      (userid,songid1,songid2,sim,rank)
    }.filter{x => x._5.toInt <2}.groupBy(_._1).map{row =>
      val songid = row._2.toArray.map{x => (x._3,x._4)}.sortBy(_._2).reverse.take(20).map{x => x._1}
      (row._1,songid.distinct)
    }
    ...
    val recLanguage3 = preUserSong.join(languageData3).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._3,x._4)}.sortBy(_._2).reverse.take(50).map{x => x._1}
      (row._1,songid.distinct)
    }
    ...
    val recBehavior6 = preUserSong.join(behaviorData6).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._3,x._4)}.sortBy(_._2).reverse.take(400).map{x => x._1}
      (row._1,songid.distinct)
    }.leftOuterJoin(recommendFilter).map{x =>
      val userid =x._1
...
//        val lastarray = lastrec.toArray
        var temp0 = 200 - firstrec.toList(0).size
        var k = firstrec.toList(0).size
