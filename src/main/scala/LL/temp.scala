...
object Fresh_song_v1_1 {
  ...
  */
  ...
  def computeList(args: Array[String], sc: SparkContext): Unit = {
    ...
    */
    val mf_parameter = "1.0_0.001_300"
    val trainDataPath = "file\\trainDataPath.txt"
    val incrementDataPath = "file\\incrementDataPath.txt"
    val playHistoryPath = "file\\playHistoryPath.txt"
    val recommendHistoryPath = "file\\recommendHistoryPath.txt"
    val songidSingerPath = "file\\songidSingerPath.txt"
    val goodSongidPath = "file\\goodSongidPath.txt"
    val badSongidPath = "file\\badSongidPath.txt"
    val similarSongPath = "file\\similarSongPath.txt"
    val specialSongPath = "file\\specialSongPath.txt"
    val songIDFPath = "file\\songIDFPath.txt"
    val userInfoPath = "file\\userInfoPath.txt"
    val productFeaturePath = "file\\productFeaturePath.txt"
    val savePath = "file\\savePath.txt"
    ...
    trainRating.foreach(println)
    val trainSongList = trainRating.map { x => (x._2, 1) }.groupBy(_._1).map { x => (x._1, 1) }
    trainSongList.foreach(println)
    trainSongList.collect()
    ...
    idfData.foreach(print)
    song_idf.foreach(println)
    song_idf = null
  ...
  def get_candidate_songlist(songid: Array[String], playdur: Array[Double], song2song: Map[String, Array[String]]): Array[String] = {
    ...
    val favorite_songid = songid.zip(playdur).sortBy(_._2).reverse.take(120).filter { x => x._2 >= 2.0 }.map { x => x._1 }
    ...
    if (candidate_songid.length > 0) {
      candidate_songid = candidate_songid.distinct.take(25000)
    }
    candidate_songid
  }
  ...
  def top5wProductFeatures(productFeatures: RDD[(String, Array[Double])]): (Array[String], DoubleMatrix) = {
    productFeatures.foreach(println)
