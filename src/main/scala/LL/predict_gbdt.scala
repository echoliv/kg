...
import org.apache.spark.mllib.linalg.{Vectors=>LibVectors,Vector=>LibVector, Matrices}
    ...
    val user_songList = sc.textFile(user_songList_path).map{ line =>
      ...
      if(group == "all"){
        ...
        try {
          ...
          if(userid.substring(lenUser-1).toInt==v1 && userid.substring(lenUser-3,lenUser-1).toInt%5==v2 | userid == "1203991582")
          {userid_select = userid}
        ...
        try {
          ...
          if(userid.substring(lenUser-1).toInt==v | userid == "1203991582")
          {userid_select = userid}
        } catch {case _: Throwable => {}}
      }
