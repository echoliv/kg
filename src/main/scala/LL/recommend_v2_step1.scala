...
object recommend_new_v2_step1 {
  ...
  def computeList(args:Array[String],sc:SparkContext): Unit ={
    ...
    val recSpecial1 = preUserSong.join(specialData1).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._2,x._3)}.sortBy(_._2).reverse.take(120).map{x => x._1}
    ...
    val recTheme2 = preUserSong.join(themeData2).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._2,x._3)}.sortBy(_._2).reverse.take(20).map{x => x._1}
      (row._1,songid.distinct)
    }
    ...
    val recLanguage3 = preUserSong.join(languageData3).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._2,x._3)}.sortBy(_._2).reverse.take(50).map{x => x._1}
      (row._1,songid.distinct)
    }
    ...
    val recAudio5 = preUserSong.join(audioData5).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._2,x._3)}.sortBy(_._2).reverse.take(10).map{x => x._1}
      (row._1,songid.distinct)
    }
