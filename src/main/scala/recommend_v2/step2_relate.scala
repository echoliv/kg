...
object recommend_new_v2_relate {
  ...
  def computeList(args:Array[String],sc:SparkContext): Unit ={
    ...
    val prerecPath = args(2)
    val recommendHistoryPath = args(3)
    val playHistoryPath = args(4)
    val outputPath1 = args(5)
    val preUserSongPath = args(6)
    val info = args(7).split('_')
    ...
    val recSpecial1 = preUserSong.join(specialData1).map{x =>
      ...
      val songid = row._2.toArray.map{x => (x._2,x._3)}.sortBy(_._2).reverse.take(maxnum).map{x => x._1}
