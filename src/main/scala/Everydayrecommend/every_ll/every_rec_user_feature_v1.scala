...
object every_rec_user_feature_v1 {
...
//    val user_feature_table = args(3)
    val songFeaturePath = "file\\everydayrecommend_test1\\song_feature"
    val favorHistoryPath = "file\\everydayrecommend_test1\\merge_playdur"
    val user_feature_table = "file\\name_7"
...
//        feature_d.toList.foreach(println)
        //132-73=59
...
//        (userId, arr_format(concat(feature_d, feature_c, feature_other)))
        (userId,feature_d.toList.toString +'|'+feature_c.toList.toString +'|'+feature_other.toList.toString)
    }
    userFeature.foreach(println)
    /*
