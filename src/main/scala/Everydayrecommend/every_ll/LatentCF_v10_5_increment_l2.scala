...
object LatentCF_increment_v3 {
  def main(args: Array[String]) {
    val conf = new SparkConf()
      .setAppName("LatentCF_increment_v3" + args(0))
  ...
  def computeList(args: Array[String], sc: SparkContext): Unit = {
    ...
    val songIDFPath = args(9)
    val userInfoPath = args(10)
    val productFeaturePath = args(11)
    val savePath = args(12)
    val mf_parameter = args(13)
    ...
    val recall_num = mf_parameter.split('_')(3).toInt
    val dislike_num = mf_parameter.split('_')(4).toInt
    ...
    // get the song to similarsong
    val similarSong = sc.textFile(similarSongPath).map { x => x.split('|') }.map { x => (x(0), x(1), x(2).toInt) }.groupBy(_._1).map { row =>
      val songid = row._2.toArray.map { x => (x._2, x._3) }.sortBy(_._2).take(recall_num).map { x => x._1 }
      (row._1, songid)
    }.join(trainSongList).map { x => (x._1, x._2._1) }

    var song2song = similarSong.groupBy(_._1).map { row =>
      val songid1 = row._1
    ...
    //get the song to similarsong for filter
    val similarSong2 = sc.textFile(similarSongPath).map { x => x.split('|') }.map { x => (x(0), x(1), x(2).toInt) }.groupBy(_._1).map { row =>
      val songid = row._2.toArray.map { x => (x._2, x._3) }.sortBy(_._2).take(dislike_num).map { x => x._1 }
      (row._1, songid)
    }.join(trainSongList).map { x => (x._1, x._2._1) }

    var song2songfilter = similarSong.groupBy(_._1).map { row =>
      val songid1 = row._1
      val songid2_list = row._2.toArray.map { x => x._2 }.flatten.distinct
      (songid1, songid2_list)
    }.collect.toMap
    val B_song2song_filter = sc.broadcast(song2songfilter)
    song2songfilter = null
    ...
    //filter the dislike song to similarsong
    val filterdislikesimilar = incrementRating.groupBy(_._1).map { row =>
      ...
      val songid = x._2._1
      val playdur = x._2._2
      val songlist = get_dislike_candidate_songlist(songid,playdur,B_song2song_filter.value)
      (userid,songlist)
    }
...
*/
    val newUserRows = incrementRating.groupBy(_._1).map{row =>
      val (indices,values) = row._2.map(e =>(e._2,e._3)).unzip
      (row._1,(indices.toArray,values.toArray))
    }.leftOuterJoin(playHistoryData).map{x => (x._1,(x._2._1._1,x._2._1._2,x._2._2))}.leftOuterJoin(recommendHistoryData).map{x =>
      (x._1,(x._2._1._1,x._2._1._2,x._2._1._3,x._2._2))
    }.leftOuterJoin(filterdislikesimilar).map{x =>
      (x._1,x._2._1._1,x._2._1._2,x._2._1._3,x._2._1._4,x._2._2)
    }.map{x =>
      val played_songid =x._4
      val recommend_songid = x._5
      val dislikesong = x._6
      var songid_to_be_filtered = new Array[String](0)
      if (played_songid != None) {
        songid_to_be_filtered = songid_to_be_filtered.union(played_songid.toList(0))
      }
      ...
      if(dislikesong !=None){
        songid_to_be_filtered = songid_to_be_filtered.union(dislikesong.toList(0))
      }
      songid_to_be_filtered = songid_to_be_filtered.distinct
      (x._1,(x._2,x._3,songid_to_be_filtered))
    }.leftOuterJoin(userInfoData).map{x =>(x._1,x._2._1._1,x._2._1._2,x._2._1._3,x._2._2)}
  ...
  //get user dislike songids
  def get_dislike_candidate_songlist(songid: Array[String], playdur: Array[Double], song2song: Map[String, Array[String]]): Array[String] = {
    var candidate_songid = new Array[String](0)
    val favorite_songid = songid.zip(playdur).sortBy(_._2).take(10).filter { x => x._2 <= 0.3 }.map { x => x._1 }
    if (favorite_songid.length > 0) {
      candidate_songid = favorite_songid.flatMap { x =>
        var tmp_list = new Array[String](0)
        if (song2song.contains(x)) {
