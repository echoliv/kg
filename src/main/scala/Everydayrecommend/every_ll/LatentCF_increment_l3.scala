//用户完整播放过歌手250召回。每个歌手召回三首,最多750首。
...
object LatentCF_increment_ll_v3 {
  def main(args: Array[String]) {
    val conf = new SparkConf()
      .setAppName("LatentCF_increment_ll_v3" + args(0))
  ...
  def computeList(args: Array[String], sc: SparkContext): Unit = {
    ...
    val singersongsimilar = args(13)
    val savePath = args(14)
    val mf_parameter = args(15)
    ...
    //get the maping from singer song to song
    var singer_s_s =sc.textFile(singersongsimilar).map{ x => x.split('|')}.map{ x => (x(0),x(1))
    } .groupBy{_._1}.map{ row =>
      val songid = row._2.map{x => (x._2)}.take(4).map{x => x}.toArray.distinct
      (row._1,songid)
    }.join(trainSongList).map{ x => (x._1,x._2._1)}.groupBy(_._1).map { row =>
      val songid1 = row._1
      val songid2_list = row._2.toArray.map { x => x._2 }.flatten.distinct
      (songid1, songid2_list)
    }.collect.toMap
    val B_singer_s_s = sc.broadcast(singer_s_s)
    singer_s_s = null
    ...
    val newRating = newuf.map { line =>
      ...
      val candidateSongid = get_candidate_songlist(songid, playdur, B_song2song.value,B_singer_s_s.value)
  ...
  def get_candidate_songlist(songid: Array[String], playdur: Array[Double], song2song: Map[String, Array[String]],singer_s_s :Map[String, Array[String]]): Array[String] = {
    var candidate_songid = new Array[String](0)
    var candidate_songid2 = new Array[String](0)
    val favorite_songid = songid.zip(playdur).sortBy(_._2).reverse.take(30).filter { x => x._2 >= 2.0 }.map { x => x._1 }
    val singer_songid = songid.zip(playdur).sortBy(_._2).reverse.take(250).filter{x=>x._2 >=2}.map {x=>x._1}
    ...
    if(singer_songid.length >0){
      candidate_songid2 = singer_songid.flatMap { x =>
        var tmp_list2 = new Array[String](0)
        if (singer_s_s.contains(x)) {
          tmp_list2 = singer_s_s(x)
        }
        tmp_list2
      }
    }
    if (candidate_songid.length > 0) {
      if (candidate_songid2.length > 0) {
        candidate_songid =candidate_songid2.union(candidate_songid).distinct.take(5000)
      } else {
