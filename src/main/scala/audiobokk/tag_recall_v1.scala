...
//      val tagfeature = x._2._2.toString.replace("Some(","").replace(")","").split('=')
      ...
      val result =get_recommend(re,B_name_album.value,B_tag_album.value,recommend_num,min_num)
      (userid+'='+result.mkString(",").replace(",null",""))
    }.saveAsTextFile(input_path)
  }

  def get_recommend(canditation: Array[(String, Double)], name_album: Map[String, String], tag_album: Map[String, String],recommend_num:Int,min_num:Int):Array[String]={
