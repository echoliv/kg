...
object A_recall_v1 {
  ...
  def computeList(args: Array[String], sc: SparkContext): Unit = {
    /*
    val user_album_path = args(0)   //用户推荐源
    val similar_album_path = args(1)  //召回池
    val user_filter_path = args(2)  //用户过滤列表
    //    val album_feature_path = args(3) //专辑特征
    val user_feature_path  = args(4)  //用户分类特征
    val tag_album_path = args(5)   //专辑_标签
    //    val time_album_path =args(6)  //专辑_发版时间
    val name_album_path =args(7)    //专辑_歌手
    val input_path = args(8)      //输出路径
    val recall_num = args(9).split('_')(0).toInt    //每个专辑召回个数
    val recommend_num =args(10).split('_')(1).toInt  //每个用户推荐个数
*/
